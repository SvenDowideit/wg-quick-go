...
func Up(cfg *Config, iface string, logger logr.Logger) error {
	log := logger.WithValues("iface", iface)
...
func Down(cfg *Config, iface string, logger logr.Logger) error {
	log := logger.WithValues("iface", iface)
...
func execSh(command string, iface string, log logr.Logger, stdin ...string) error {
	cmd := exec.Command("sh", "-ce", strings.ReplaceAll(command, "%i", iface))
	if len(stdin) > 0 {
		log = log.WithValues("stdin", strings.Join(stdin, ""))
		b := &bytes.Buffer{}
	...
	if err != nil {
		log.Error(err, "failed to execute %s:\n%s", cmd.Args, out)
		return err
	}
	log.Info("executed", "args", cmd.Args, "output", out)
	return nil
}
...
func Sync(cfg *Config, iface string, logger logr.Logger) error {
	log := logger.WithValues("iface", iface)

	link, err := SyncLink(cfg, iface, log)
	if err != nil {
		log.Error(err, "cannot sync wireguard link")
		return err
	}
...
func SyncWireguardDevice(cfg *Config, link netlink.Link, log logr.Logger) error {
	cl, err := wgctrl.New()
	if err != nil {
	...
	if err := cl.ConfigureDevice(link.Attrs().Name, cfg.Config); err != nil {
		log.Error(err, "cannot configure device")
		return err
	}
	return nil
}
...
func SyncLink(cfg *Config, iface string, log logr.Logger) (netlink.Link, error) {
	link, err := netlink.LinkByName(iface)
	if err != nil {
		if _, ok := err.(netlink.LinkNotFoundError); !ok {
		...
		if err := netlink.LinkAdd(wgLink); err != nil {
			log.Error(err, "cannot create link")
			return nil, err
		}
	...
	if err := netlink.LinkSetUp(link); err != nil {
		log.Error(err, "cannot set link up")
		return nil, err
	}
...
func SyncAddress(cfg *Config, link netlink.Link, log logr.Logger) error {
	...
	for _, addr := range cfg.Address {
		log := log.WithValues("addr", addr.String())
...
func SyncRoutes(cfg *Config, link netlink.Link, managedRoutes []net.IPNet, log logr.Logger) error {
	...
	for _, rt := range managedRoutes {
		rt := rt // make copy
		log.WithValues("dst", rt.String()).V(1).Info("managing route")
	...
	for _, rt := range presentRoutes {
		...
		if checkWanted(rt) {
			log.V(1).Info("route wanted, skipping deleting")
			continue
		}

		if err := netlink.RouteDel(&rt); err != nil {
			log.Error(err, "cannot delete route")
			return err
		}
